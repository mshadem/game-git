...
HT.Grid = function(/*double*/ width, /*double*/ height) {
	...
	var row = 0;
	var y = GRID_Y_PADDING;
	while (y + HT.Hexagon.Static.HEIGHT <= height)
	{
		var col = 0;

		var offset = GRID_X_PADDING;
		if (row % 2 == 1)
		{
			if(HT.Hexagon.Static.ORIENTATION == HT.Hexagon.Orientation.Normal)
				offset = (HT.Hexagon.Static.WIDTH - HT.Hexagon.Static.SIDE)/2 + HT.Hexagon.Static.SIDE + GRID_X_PADDING;
			else
				offset = HT.Hexagon.Static.WIDTH / 2 + GRID_X_PADDING;
			col = 1;
		}
...
HT.Grid.prototype.GetWidth = function () {
  var MaxX = 0;
  for (i in this.Hexes)  
    if (this.Hexes[i].PathCoOrdX > MaxX)
      MaxX = this.Hexes[i].PathCoOrdX;
  return MaxX;      
};

HT.Grid.prototype.GetHeight = function () {
  var MaxY = 0;
  for (i in this.Hexes)  
    if (this.Hexes[i].PathCoOrdY > MaxY)
      MaxY = this.Hexes[i].PathCoOrdY;
  return MaxY;      
};